...
export class MetricsStore {
  ...
  public async updateDailyMetric(
    updateFn: (metrics: DailyMetrics) => void,
  ): Promise<void> {
    const today = this.getTodayDate();
    const allMetrics = this.context.globalState.get<
      Record<string, DailyMetrics>
    >(MetricsStore.STORAGE_KEY, {});
  ...
  public async incrementCounter(
    metric: keyof Omit<
      DailyMetrics,
      "date" | "filesEdited" | "languages" | "projects"
    >,
    amount: number = 1,
  ): Promise<void> {
  ...
  public getAggregatedMetrics(
    year: number,
    month?: number,
    timeRange?: "daily" | "weekly" | "monthSoFar" | "monthly" | "yearly",
  ): AggregatedMetrics {
    console.log(
      `[MetricsStore] Aggregating metrics for Year: ${year}, Month: ${month}, TimeRange: ${timeRange || "yearly"}`,
    );
    const allMetrics = this.context.globalState.get<
      Record<string, DailyMetrics>
    >(MetricsStore.STORAGE_KEY, {});
    ...
    for (const dateStr of relevantDates) {
      ...
      console.log("dateStr:", dateStr, "dayMetrics:", dayMetrics);
      ...
      aggregated.totalIntelliSenseAccepts +=
        dayMetrics.intellisenseAccepts || 0;
      ...
      if (
        (dayMetrics.longestSessionMinutes || 0) >
        aggregated.longestSessionMinutes
      ) {
        aggregated.longestSessionMinutes =
          dayMetrics.longestSessionMinutes || 0;
      }
      ...
      Object.entries(dayMetrics.hourlyActivity || {}).forEach(
        ([hour, count]) => {
          hourlyActivityCounts[Number(hour)] =
            (hourlyActivityCounts[Number(hour)] || 0) + count;
        },
      );

      // Aggregate day of week activity
      Object.entries(dayMetrics.dayOfWeekActivity || {}).forEach(
        ([day, count]) => {
          dayOfWeekActivityCounts[Number(day)] =
            (dayOfWeekActivityCounts[Number(day)] || 0) + count;
        },
      );
      ...
      (Object.entries(dayMetrics.languages) as [string, number][]).forEach(
        ([lang, count]) => {
    ...
    aggregated.netLines =
      aggregated.totalLinesAdded - aggregated.totalLinesDeleted;
    aggregated.lateNightSessionCount = lateNightSessionCount;
    aggregated.totalLateNightMinutes = totalLateNightMinutes;
    aggregated.weekendCodingPercentage =
      aggregated.totalCodingTimeMinutes > 0
        ? Math.round(
            (totalWeekendMinutes / aggregated.totalCodingTimeMinutes) * 100,
          )
        : 0;

    // Find most productive hour
    const maxHourActivity = Math.max(...Object.values(hourlyActivityCounts), 0);
    aggregated.mostProductiveHour =
      maxHourActivity > 0
        ? Number(
            Object.keys(hourlyActivityCounts).find(
              (h) => hourlyActivityCounts[Number(h)] === maxHourActivity,
            ),
          )
        : null;
    ...
    const maxDayActivity = Math.max(
      ...Object.values(dayOfWeekActivityCounts),
      0,
    );
    aggregated.mostProductiveDay =
      maxDayActivity > 0
        ? Number(
            Object.keys(dayOfWeekActivityCounts).find(
              (d) => dayOfWeekActivityCounts[Number(d)] === maxDayActivity,
            ),
          )
        : null;
    ...
    aggregated.dailyHistory = relevantDates.map((dateStr) => {
      ...
      return {
        date: dateStr,
        count:
          (m.totalEdits || 0) + (m.linesAdded || 0) + (m.linesDeleted || 0),
      };
    });
  ...
  public getAllRawMetrics(): Record<string, DailyMetrics> {
    return this.context.globalState.get<Record<string, DailyMetrics>>(
      MetricsStore.STORAGE_KEY,
      {},
    );
  }
